...
<tr><td align=right><font face=arial size=4><b><%=teste.descricao%></b></font></td></tr>
<tr><td align=right><font face=arial size=2><b>Código do exame: </b><%=cod_Exame%></font></td></tr>
<tr><td align=left><font face=arial size=2><b>Área objetivo do exame: </b><%=teste.area_texto%></font></td></tr>
<tr><td align=left><font face=arial size=2><b>Período do exame: </b><%=teste.ano & ", " & teste.semestre%>o semestre</font></td></tr>

<%  if teste.provas.count < 4 then%>

	<% if teste.provas.count = 0 then%>

			<tr><td align=right><font face=arial size=1><a style="color=blue;" href="javascript:adicionar_prova();"  >Adiciona Prova ao EXAME</a> <a style="color=blue;"  href=operacao.asp?origem=EXAME&comando=APAGA&cod_conf_exame=<%=cod_exame%> >REMOVE EXAME</a></td></tr>	
...
%>
			<tr><td align=left><font face=arial size=2>Prova: <b><%=item.descricao%></b></font></td></tr>
			<tr><td align=left><font face=arial size=2>Código de configuração da prova: <%=item.cod_conf_prova%> </font></td></tr>
			<tr><td align=left><font face=arial size=2>Tempo mínimo para realização da prova: <%=item.tempomin%> minutos</font></td></tr>
			<tr><td align=left><font face=arial size=2>Tempo máximo para realização da prova: <%=item.tempomax%> minutos</font></td></tr>
			<tr><td align=left><font face=arial size=2>Total de questões desta prova: <%=total_quest%> </font></td></tr>
			
			<% if item.disciplinas.count < 5 then%>
			 
				<tr ><td align=right><font face=arial size=1><a style="color=blue;"  href="javascript:adicionar(<%=contador%>, <%=item.cod_conf_prova%>);">Adiciona disciplina à prova <%=item.descricao%></a> <% if item.disciplinas.count = 0 then %> <a style="color=blue;"  href=operacao.asp?origem=PROVA&comando=APAGA&cod_conf_prova=<%=item.cod_conf_prova%>>Remover prova <%=item.descricao%></a><% end if %></td></tr>			
		...
		<%	For Each disc In item.disciplinas %>
				...
				<tr ><td valign=top width=30%><font face=arial size=2><b> <u><%=disc.disciplina_texto%></u></b></td>
				<td width=70%>
	...
	<tr><td align=left><font face=arial size=2><a style="color=blue;"  href=default.asp>Voltar</a></td></tr>
